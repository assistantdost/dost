...
flowchart TD
    subgraph Query Path
        UQ1[User Query] --> V{Vague?}
        V -- yes --> G[_getLastUserQuery] --> EQ[Enriched Query]
        V -- no --> OQ[Original Query]
        EQ --> EMB1[Embed]
        OQ --> EMB1
        EMB1 --> VS[Vector similarity] --> SH[Semantic Hits]
    end

    subgraph Keyword Path
        UQ2[User Query] --> TS[BM25 Text Search] --> TH[Text Hits]
    end

    subgraph Sticky Path
        CH[Conversation History] --> SE[Sticky Extraction] --> ST[Sticky Tools]
    end

    SH --> MD[Merge & Deduplicate]
    TH --> MD
    ST --> MD
    MD --> FTO[Filtered Tools Object] --> LA[LLM Agent]
```
...
```mermaid
flowchart TD
    Start([index activeTools]) --> CheckInput{Is tools list empty?}
    CheckInput -- Yes --> ReturnEmpty[Return 0 count]
    CheckInput -- No --> InitOrama[Initialize new Orama database instance]
    InitOrama --> LoopStart[Loop through each Tool definition]
    
    subgraph Enrichment Pipeline
        LoopStart --> ExtParams[Extract parameter names from input schema]
        ExtParams --> GenVars[Split name into individual words nameVariations]
        GenVars --> BuildText[Construct embedText:<br>name + nameVariations + description + paramNames]
        BuildText --> GenEmbed[Generate 384-dim normalized vector via MiniLM model]
    end

    GenEmbed --> InsertDB[Insert document into Orama DB:<br>name, desc, schemaText, nameVariations, embedding]
    InsertDB --> CheckNext{Has more tools?}
    CheckNext -- Yes --> LoopStart
    CheckNext -- No --> ReturnResult([Return total count and time elapsed])
```
...
```mermaid
flowchart TD
    Start([select tools]) --> CheckPipeline{ToolRAG ready & db indexed?}
    CheckPipeline -- No --> ReturnAll[Return all tools fallback]
    CheckPipeline -- Yes --> RunSemantic[Run semantic search on original query]
    
    RunSemantic --> VagueCheck{Is query vague?<br>words <= 6 OR top score < 0.4}
    
    VagueCheck -- No --> RunText[Run BM25 keyword textSearch]
    
    VagueCheck -- Yes --> GetHistory[Fetch previous user message from history]
    GetHistory --> HasHistory{Context found?}
    
    HasHistory -- No --> RunText
    
    HasHistory -- Yes --> EnrichQuery[Construct enriched query:<br>query + Context: prevQuery]
    EnrichQuery --> RunEnrichedSemantic[Run semantic search on enriched query]
    RunEnrichedSemantic --> MergeSemantic[Merge original and enriched hits]
    MergeSemantic --> RunText
    
    RunText --> ExtractSticky[Extract sticky tools from K turns of history]
    ExtractSticky --> UnionSets[Combine hits:<br>Semantic + Text + Sticky]
    UnionSets --> FilterMCP[Filter matching tools from allTools map]
    FilterMCP --> End([Return filtered tools object])
```
...
```mermaid
flowchart TD
    Start([_generateNameVariations]) --> Init[Initialize unique Set 'words']
    Init --> SplitSeps[Split name by separators:<br>underscore, hyphen, space]
    SplitSeps --> LoopSeps[Loop word segments]
    LoopSeps --> FilterEmpty{Length > 0?}
    FilterEmpty -- Yes --> AddSepSet[Add to Set]
    FilterEmpty -- No --> NextSep{More segments?}
    AddSepSet --> NextSep
    
    NextSep -- Yes --> LoopSeps
    NextSep -- No --> SplitCamel[Split name by uppercase boundaries:<br>camelCase / PascalCase]
    
    SplitCamel --> LoopCamel[Loop camel segments]
    LoopCamel --> FilterEmpty2{Length > 0?}
    FilterEmpty2 -- Yes --> AddCamelSet[Add to Set]
    FilterEmpty2 -- No --> NextCamel{More segments?}
    AddCamelSet --> NextCamel
    
    NextCamel -- Yes --> LoopCamel
    NextCamel -- No --> ReturnArray[Convert Set to Array & Return]
```
...
```mermaid
flowchart TD
    Start([_getLastUserQuery]) --> Init[Set userTurnsSeen = 0]
    Init --> LoopStart[Loop messages from last to first]
    LoopStart --> CheckUser{Is role 'user'?}
    CheckUser -- No --> CheckNext{Has more messages?}
    CheckUser -- Yes --> Increment[Increment userTurnsSeen]
    
    Increment --> IsFirstUser{Is userTurnsSeen < 2?}
    IsFirstUser -- Yes --> CheckNext[Skip current query]
    IsFirstUser -- No --> ExtractContent{Is msg.content an array?}
    
    ExtractContent -- Yes --> ExtractArray[Filter text type blocks & join]
    ExtractContent -- No --> ExtractString[Return msg.content string trimmed]
    
    ExtractArray --> CheckEmpty{Is result text empty?}
    CheckEmpty -- No --> ReturnText[Return Context string]
    CheckEmpty -- Yes --> CheckNext
    
    ExtractString --> CheckStringEmpty{Is result text empty?}
    CheckStringEmpty -- No --> ReturnText2[Return Context string]
    CheckStringEmpty -- Yes --> CheckNext
